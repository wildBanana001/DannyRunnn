...
2. 用户到店后，管理员从小程序“我的 → 待核销订单”确认商品已经实际交付；这个入口只对 `ADMIN_OPENID_WHITELIST` 中的微信 OpenID 展示，接口仍会在服务端再次鉴权。
3. 管理员点击“确认交付并上报”后，BFF 原子记录核销人和时间，再调用微信小程序订单发货管理接口，以 `logistics_type=4`（用户自提）上报，不需要再进入微信后台手工选择自提和发货。
4. 微信接口失败时，门店交付记录不会丢失，订单会继续留在待核销页面并显示“重试同步微信”。并发请求、接口超时和重复上报均按订单号幂等处理。
...
管理员首次配置时，登录小程序并在“我的”页面 1.2 秒内连续点击底部“我的”Tab 三次，即可查看并复制当前账号的 OpenID。把这个值加入云托管服务变量 `ADMIN_OPENID_WHITELIST`（多人用英文逗号分隔），然后发布新版本或重启服务让白名单生效。不要把管理员 OpenID 写死在小程序前端。
...
- `GET /api/admin-mini/fulfillment-tasks`：读取管理员待办，包括已支付待到店核销订单，以及已经交付但微信上报失败、需要重试的订单
- `POST /api/admin-mini/fulfillment-tasks/:kind/:id/complete`：统一完成活动到场或商品到店交付，并幂等上报微信“用户自提”；`kind` 为 `activity` 或 `shop`
