...
export interface WechatSelfPickupShippingPayload {
  order_key: {
    order_number_type: 1;
    mchid: string;
    out_trade_no: string;
...
export class WechatOrderShippingApiError extends Error {
  ...
  constructor(code: number | string, message: string, retriable = false) {
    super(message);
...
function assertWechatOrderShippingConfigured() {
  ...
  if (issues.length > 0) {
    throw new WechatOrderShippingApiError(
      'CONFIGURATION_REQUIRED',
      `微信订单履约上报配置不完整：${issues.join('、')}`,
    );
  }
}
...
async function fetchAccessToken(forceRefresh = false): Promise<string> {
  ...
  if (!forceRefresh && accessTokenRequest) return accessTokenRequest;

  assertWechatOrderShippingConfigured();
  const request = (async () => {
    const response = await axios.get<WechatApiResponse & { access_token?: string; expires_in?: number }>(
      ACCESS_TOKEN_URL,
      {
        params: {
    ...
    if (!response.data.access_token) {
      throw new WechatOrderShippingApiError(
        response.data.errcode ?? 'ACCESS_TOKEN_FAILED',
        response.data.errmsg || '获取微信 access_token 失败',
      );
    }
    accessTokenCache = {
...
async function postShippingInfo(payload: WechatSelfPickupShippingPayload, forceRefreshToken = false) {
  const accessToken = await fetchAccessToken(forceRefreshToken);
  const response = await axios.post<WechatApiResponse>(UPLOAD_SHIPPING_URL, payload, {
...
export async function uploadWechatSelfPickupShippingInfo(order: OrderRecord) {
  assertWechatOrderShippingConfigured();
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      if (error instanceof WechatOrderShippingApiError) throw error;
    ...
    if (errorCode === 0 || IDEMPOTENT_SUCCESS_ERROR_CODES.has(errorCode)) return;
    ...
    throw new WechatOrderShippingApiError(
      errorCode,
      result.errmsg || `微信订单履约上报失败（${errorCode}）`,
      RETRIABLE_ERROR_CODES.has(errorCode),
    );
  }
