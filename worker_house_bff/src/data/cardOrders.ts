...
interface UpdateCardOrderInput {
  expiresAt?: string;
  reason?: string;
...
function resolveCardOrderStatus(status: unknown, remainingCount: number, expiresAt?: string): CardOrderStatus {
...
function normalizeUsageLog(record: Partial<CardUsageLog>): CardUsageLog {
  return {
...
function normalizeAdjustLog(record: Partial<CardOrderAdjustLog>): CardOrderAdjustLog {
  ...
  return {
    ...
    from: {
      ...
      status: sanitizeString(record.from?.status) as CardOrderStatus || undefined,
    },
    ...
    to: {
      ...
      status: sanitizeString(record.to?.status) as CardOrderStatus || undefined,
    },
  };
}

function normalizeCardOrder(record: Partial<CardOrder>): CardOrder {
...
function loadCardOrders() {
  ...
  try {
    ...
    const parsed = JSON.parse(rawContent) as Partial<CardOrder>[];
...
function isOrderUsable(order: CardOrder) {
...
export function listCardOrdersByOpenid(openid: string) {
  loadCardOrders();
...
export function getCardOrderById(openid: string, cardOrderId: string) {
  loadCardOrders();
...
export function getCardOrderByIdUnsafe(cardOrderId: string) {
  loadCardOrders();
  const record = memoryStore.cardOrders.find((item) => item.id === cardOrderId) ?? null;
  return clone(record ? normalizeCardOrder(record) : null);
}

export function getUsageLogsByCardOrderId(openid: string, cardOrderId: string) {
  const order = getCardOrderById(openid, cardOrderId);
  return order ? clone(sortUsageLogs(order.usageLogs)) : null;
}

export function findUsableCardOrder(openid: string, profileId: string, cardOrderId?: string) {
  loadCardOrders();

  if (cardOrderId) {
    const exact = memoryStore.cardOrders.find(
      (item) => item.openid === openid && item.profileId === profileId && item.id === cardOrderId,
    );
    return exact && isOrderUsable(exact) ? clone(normalizeCardOrder(exact)) : null;
  }

  const usable = sortOrders(
    memoryStore.cardOrders.filter(
      (item) => item.openid === openid && item.profileId === profileId && isOrderUsable(item),
    ),
  );
...
export function applyCardUsage(input: ApplyCardUsageInput) {
  loadCardOrders();
...
export function updateCardOrderStatus(cardOrderId: string, status: CardOrderStatus) {
...
export function updateCardOrder(cardOrderId: string, input: UpdateCardOrderInput) {
  loadCardOrders();
