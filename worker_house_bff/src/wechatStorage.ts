...
function getErrorMessage(error: unknown) {
  if (axios.isAxiosError(error)) {
    const payload = error.response?.data as { errmsg?: string; error?: string; message?: string } | undefined;
...
async function parseWechatResponse<T extends WechatApiErrorPayload>(response: Response) {
...
async function withRetry<T>(label: string, executor: () => Promise<T>, retries = 2) {
...
function isTokenExpiredError(error: unknown) {
  ...
  const payload = error.response?.data as { errcode?: number; errmsg?: string } | undefined;
...
function getContentType(fileName: string, preferred?: string) {
...
export function buildCloudStoragePath(scope: string, fileName: string) {
...
async function requestUploadTicket(cloudPath: string): Promise<UploadTicket> {
...
async function uploadFileToCos(cloudPath: string, ticket: UploadTicket, source: UploadFileSource) {
  return withRetry('上传文件到云存储', async () => {
    ...
    form.append(
      'file',
      new Blob([new Uint8Array(source.buffer)], {
...
async function getDownloadUrl(fileID: string, maxAge = 31536000) {
  const accessToken = await getAccessToken();
  ...
  return withRetry('获取文件下载链接', async () => {
    ...
    const payload = await parseWechatResponse<{
      errcode?: number;
      errmsg?: string;
      file_list?: Array<{ download_url?: string; errmsg?: string; status?: number }>;
    }>(
      await fetch(url, {
        method: 'POST',
        headers: {
...
export async function uploadToWechatCloudStorage(
  ...
  cloudPath = buildCloudStoragePath('admin', source.fileName),
): Promise<UploadedFileResult> {
  const safeFileName = sanitizeFileName(source.fileName);
  const normalizedSource: UploadFileSource = {
    ...source,
