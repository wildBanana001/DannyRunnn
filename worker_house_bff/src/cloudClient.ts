...
export interface CloudFunctionSuccess<T> {
  success: true;
  data: T;
}
...
function fail(error: string): CloudFunctionFailure {
  return { success: false, error };
}

function success<T>(data: T): CloudFunctionSuccess<T> {
  return { success: true, data };
}

function parseRespData(value: unknown) {
...
async function invokeWechatCloudFunction<T>(name: CloudFunctionName, event: Record<string, unknown>) {
  ...
  const parsed = parseRespData(payload.resp_data) as T | CloudFunctionResult<T>;

  if (parsed && typeof parsed === 'object' && 'success' in parsed) {
    return parsed as CloudFunctionResult<T>;
  }

  return success(parsed as T);
}

function toAuthUserFromAdmin(admin: Pick<AdminRecord, 'id' | 'name' | 'role' | 'username'>): AuthUser {
  return {
    id: admin.id,
    name: admin.name,
    role: admin.role,
...
function createFallbackAdminUser(token: string): AuthUser {
  return {
...
function getMockAdminUserByToken(token: string) {
  const admin = mockStore.getAdminByToken(token);
  return admin ? toAuthUserFromAdmin(admin) : null;
}

function registerAdminSession(token: string, user: AuthUser) {
...
export function normalizeActivity(record: Record<string, unknown> | null | undefined): ActivityRecord | null {
  if (!record) {
    return null;
  }
  ...
  const covers = Array.from(
    new Set(
      [cover, coverImage, ...(Array.isArray(record.covers) ? record.covers.map((item) => String(item)) : []), ...gallery]
  ...
  return {
    ...
    originalPrice:
      record.originalPrice === undefined || record.originalPrice === null
        ? undefined
    ...
    status: (record.status as ActivityRecord['status']) ?? 'upcoming',
    ...
    signups: Array.isArray(record.signups)
      ? (record.signups as ActivityRecord['signups'])
      : [],
    createdAt: String(record.createdAt ?? new Date().toISOString()),
    updatedAt: String(record.updatedAt ?? record.createdAt ?? new Date().toISOString()),
  };
...
export function normalizePost(record: Record<string, unknown> | null | undefined): PostRecord | null {
  if (!record) {
    return null;
  }
  ...
  return {
    ...
    tags: Array.isArray(record.tags) ? record.tags.map((item) => String(item)) : [],
    color: record.color as PostRecord['color'],
    isPinned: pinned,
    pinned,
    createdAt: String(record.createdAt ?? new Date().toISOString()),
    updatedAt: String(record.updatedAt ?? record.createdAt ?? new Date().toISOString()),
...
export function normalizeComment(record: Record<string, unknown> | null | undefined): CommentRecord | null {
  if (!record) {
    return null;
  }
...
export function normalizeSiteConfig(record: Record<string, unknown> | null | undefined): SiteConfigRecord {
  return {
...
export function toCloudSiteConfigPayload(data: SiteConfigRecord) {
  return {
    ...data,
...
function invokeMockCloudFunction<T>(name: CloudFunctionName, event: Record<string, unknown>) {
  switch (name) {
    case 'admin_auth': {
      if (event.action !== 'login') {
        return fail('仅支持 login') as CloudFunctionResult<T>;
      }

      const admin = mockStore.loginAdmin(String(event.username ?? ''), String(event.password ?? ''));
      if (!admin) {
        return fail('账号或密码错误') as CloudFunctionResult<T>;
      }

      return success({ token: admin.token, username: admin.username } as T);
    }
    case 'poster': {
      ...
      if (action === 'list') {
        ...
        return success(records as T);
      }

      if (action === 'get') {
        const record = mockStore.getPoster(String(event.id ?? ''));
        return record ? success(record as T) : fail('海报不存在');
      }

      if (action === 'create') {
        const record = mockStore.createPoster(
          event.data as Omit<PosterRecord, 'id' | 'createdAt' | 'updatedAt'>,
        );
        return success({ id: record.id } as T);
      }

      if (action === 'update') {
        const record = mockStore.updatePoster(String(event.id ?? ''), event.data as PosterRecord);
        return record ? success({ id: record.id } as T) : fail('海报不存在');
      }

      if (action === 'delete') {
        const deleted = mockStore.deletePoster(String(event.id ?? ''));
        return deleted ? success({ id: event.id } as T) : fail('海报不存在');
      }

      if (action === 'reorder') {
        mockStore.reorderPosters(Array.isArray(event.ids) ? (event.ids as string[]) : []);
        return success({ success: true } as T);
      }
    ...
    case 'activity': {
      const action = event.action;
      if (action === 'list') {
        return success(mockStore.listActivities() as T);
      }

      if (action === 'get') {
        const record = mockStore.getActivity(String(event.id ?? ''));
        return record ? success(record as T) : fail('活动不存在');
      }
      ...
      if (action === 'signup') {
        ...
        return record ? success({ id: record.id } as T) : fail('活动不存在');
      }

      if (action === 'create') {
        const record = mockStore.createActivity(
          event.data as Omit<ActivityRecord, 'id' | 'createdAt' | 'updatedAt'>,
        );
        return success({ id: record.id } as T);
      }

      if (action === 'update') {
        const record = mockStore.updateActivity(String(event.id ?? ''), event.data as ActivityRecord);
        return record ? success({ id: record.id } as T) : fail('活动不存在');
      }
    ...
    case 'site_config': {
      const action = event.action;
      if (action === 'get') {
        return success(mockStore.getSiteConfig() as T);
      }

      if (action === 'update') {
        mockStore.updateSiteConfig(normalizeSiteConfig(event.data as Record<string, unknown>));
        return success({ id: 'site-config' } as T);
      }

      return fail(`不支持的 action: ${String(action)}`) as CloudFunctionResult<T>;
    }
    default:
      return fail(`不支持的云函数: ${name}`) as CloudFunctionResult<T>;
  }
}
...
export async function loginAdmin(username: string, password: string) {
  const result = await callCloudFunction<{ token: string; username: string }>('admin_auth', {
    action: 'login',
    password,
    username,
  });
