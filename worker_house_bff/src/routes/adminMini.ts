...
function compareDateDesc(firstValue: string, secondValue: string) {
  return new Date(secondValue).getTime() - new Date(firstValue).getTime();
}

function buildPagedResult<T>(records: T[], page: number, pageSize: number) {
...
function normalizeRegistrationStatus(value: unknown) {
  const normalizedValue = sanitizeString(value) as RegistrationStatus;
  return registrationStatuses.has(normalizedValue) ? normalizedValue : null;
}

function normalizeCardOrderStatus(value: unknown) {
  const normalizedValue = sanitizeString(value) as CardOrderStatus;
  return cardOrderStatuses.has(normalizedValue) ? normalizedValue : null;
}

function normalizeAdminMiniPost(record: Record<string, unknown>) {
...
function buildPaymentRegistration(order: OrderRecord | null): Registration | null {
  if (!order) return null;
  const snapshot = order.activityRegistration;
  if (order.kind !== 'activity' || !snapshot) return null;
...
async function fetchAdminMiniPosterDetail(id: string) {
  const result = await callCloudFunction<Record<string, unknown>>('poster', {
    action: 'get',
    id,
  });
...
adminMiniRouter.get('/registrations', async (request, response) => {
  try {
    ...
    const paymentRecords = (await getOrdersByKind('activity'))
      ...
      .filter((item): item is Registration => Boolean(item));
...
adminMiniRouter.post('/card-packages', (request, response) => {
  const payload = request.body as Partial<CardPackage>;
...
adminMiniRouter.put('/card-packages/:id', (request, response) => {
  ...
  const record = updateCardPackage(current.id, request.body as Partial<CardPackage>);
...
adminMiniRouter.post('/posters', async (request, response) => {
  try {
    const payload = buildPosterPayload((request.body as Record<string, unknown>) ?? {});
...
adminMiniRouter.put('/posters/:id', async (request, response) => {
  try {
    ...
    const payload = buildPosterPayload((request.body as Record<string, unknown>) ?? {}, current.data);
    const result = await callCloudFunction<{ id: string }>('poster', {
      action: 'update',
      id: String(request.params.id),
      data: payload,
