...
import {
  callCloudFunction,
  normalizeSiteConfig,
  toCloudSiteConfigPayload,
} from '../cloudClient.js';
import { authMiddleware, resolveRequestToken } from '../middleware/auth.js';

async function fetchSiteConfig() {
  const result = await callCloudFunction<Record<string, unknown> | null>('site_config', {
    action: 'get',
  });
...
siteRouter.put('/config', authMiddleware, async (request, response) => {
  try {
    const payload = normalizeSiteConfig(request.body as Record<string, unknown>);
    const result = await callCloudFunction<{ id: string }>('site_config', {
      action: 'update',
