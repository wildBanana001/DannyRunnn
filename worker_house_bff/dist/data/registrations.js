...
export function calculateCardOffset({ cardEligible, maxOffset, price, remaining, useCard, }) {
    if (!useCard || !cardEligible || remaining <= 0) {
        return 0;
    }
    return Math.min(price, Math.max(0, maxOffset));
}
function listRegistrationsByOpenidUnsafe(filters = {}) {
    loadRegistrations();
...
loadRegistrations();
export function listRegistrationsByOpenid(openid, filters = {}) {
    loadRegistrations();
...
export function listAllRegistrations(filters = {}) {
    return clone(sortRegistrations(listRegistrationsByOpenidUnsafe(filters)));
}
export function getRegistrationById(openid, registrationId) {
    loadRegistrations();
...
export function getRegistrationByIdUnsafe(registrationId) {
    loadRegistrations();
...
export function createRegistration(openid, input) {
    loadRegistrations();
