...
export function getAddressesByOpenid(openid) {
    loadAddresses();
...
export function createAddress(openid, payload) {
    loadAddresses();
    const normalizedOpenid = sanitizeString(openid);
    if (!normalizedOpenid)
...
export function updateAddress(openid, id, payload) {
    loadAddresses();
    const normalizedOpenid = sanitizeString(openid);
    if (!normalizedOpenid)
        throw new Error('openid 不能为空');
...
export function deleteAddress(openid, id) {
    loadAddresses();
    const normalizedOpenid = sanitizeString(openid);
    if (!normalizedOpenid)
        throw new Error('openid 不能为空');
