...
function normalizeProfile(openid, input, current) {
    const timestamp = now();
...
export function updateProfile(openid, profileId, input) {
    const currentProfiles = memoryStore.profiles.filter((item) => item.openid === openid);
...
export function deleteProfile(openid, profileId) {
    const currentProfiles = memoryStore.profiles.filter((item) => item.openid === openid);
...
export function setDefaultProfile(openid, profileId) {
    const currentProfiles = memoryStore.profiles.filter((item) => item.openid === openid);
...
export function resolveProfileForRegistration(openid, profileId) {
    if (profileId) {
