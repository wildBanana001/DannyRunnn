...
function deriveActivityStatus(startDate, endDate) {
...
function normalizeActivityRecord(record) {
    ...
    const covers = Array.from(new Set([cover, coverImage, ...(record.covers ?? []), ...(record.gallery ?? [])].map((item) => sanitizeString(item)).filter(Boolean)));
...
function buildActivityRecord(input, current) {
...
export function upsertActivity(activityId, input) {
    loadActivities();
...
export function registerActivityParticipant(activityId, signup) {
    loadActivities();
