...
function normalizeCode(value, detail) {
    ...
    if (/ER_NO_SUCH_TABLE|doesn't exist|table.*not found/i.test(detail))
        return 'ER_NO_SUCH_TABLE';
    if (/ER_BAD_DB_ERROR|unknown database/i.test(detail))
        return 'ER_BAD_DB_ERROR';
    if (/ER_ACCESS_DENIED_ERROR|access denied/i.test(detail))
        return 'ER_ACCESS_DENIED_ERROR';
    if (/fetch failed|network|ECONN|ENOTFOUND|ETIMEDOUT/i.test(detail))
        return 'NETWORK_FETCH_FAILED';
    if (/mysql|database|数据库/i.test(detail))
        return 'MYSQL_STORAGE_ERROR';
...
function normalizeDetail(code, detail) {
    if (code === 'MYSQL_CONFIGURATION_REQUIRED')
        return '微信云托管 MySQL 配置不完整。请配置 MYSQL_ADDRESS、MYSQL_USERNAME、MYSQL_PASSWORD、MYSQL_DATABASE。';
    if (code === 'ER_ACCESS_DENIED_ERROR')
        return 'MySQL 用户名或密码无效，请核对云托管 MySQL 连接信息。';
    if (code === 'ER_BAD_DB_ERROR')
        return 'MySQL 数据库不存在，请先在微信云托管的 MySQL 页面创建数据库，并核对 MYSQL_DATABASE。';
    if (code === 'ER_NO_SUCH_TABLE')
        return 'MySQL 订单表尚未初始化，请保持 MYSQL_AUTO_MIGRATE=true 后重新部署，或执行 npm run migrate:orders。';
...
function inferSource(error, stage, code, detail) {
    ...
    if (stage === 'order_lookup' || stage === 'order_create' || stage === 'order_settle')
        return 'mysql';
    if (/mysql|database|数据库|^ER_|ECONN|ENOTFOUND|ETIMEDOUT/i.test(`${code} ${detail}`))
        return 'mysql';
    return 'bff';
}
function isStorageUnavailable(code) {
    return [
