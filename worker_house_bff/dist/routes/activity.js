...
async function fetchActivityDetail(id) {
    const detailResult = await callCloudFunction('activity', {
        action: 'get',
        id,
    });
...
activityRouter.get('/', async (request, response) => {
    ...
    try {
        const result = await callCloudFunction('activity', {
            action: 'list',
            status,
        });
        if (!result.success) {
        ...
        list = list.sort((first, second) => new Date(second.createdAt).getTime() - new Date(first.createdAt).getTime());
