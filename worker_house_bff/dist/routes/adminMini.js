...
import { confirmActivityOrderFulfillment, confirmShopOrderFulfillment, OrderFulfillmentError, } from '../services/order-fulfillment.js';
...
function toAdminShopOrder(order) {
    return {
        id: order.id,
        openid: order.openid,
...
adminMiniRouter.put('/site-config', (request, response) => {
    try {
        ...
        const updated = updateSiteConfig({
            ...
            homeSpaceImages: Array.isArray(body.homeSpaceImages) ? body.homeSpaceImages : undefined,
            homeOwners: Array.isArray(body.homeOwners)
                ? body.homeOwners.map((item, index) => ({
...
adminMiniRouter.post('/shop-orders/:id/fulfill', async (request, response) => {
    try {
        const order = await confirmShopOrderFulfillment(String(request.params.id), resolveAdminOpenid(request));
...
adminMiniRouter.patch('/registrations/:id/status', async (request, response) => {
    ...
    if (paymentOrder?.kind === 'activity') {
        ...
        try {
            const fulfilled = await confirmActivityOrderFulfillment(paymentOrder.id, resolveAdminOpenid(request));
