...
import { buildPaymentFailureResponse, } from '../utils/payment-diagnostics.js';
...
export function resolveShopOrderAddress(fulfillmentType, value) {
...
function matchesActivityPaymentDetails(order, input) {
...
async function closePendingOrderSafely(order, failureReason, requireExpired = true) {
    let latest = await refreshWechatOrder(order);
    if (latest.status !== 'pending' || (requireExpired && !isExpired(latest)))
        return latest;
        ...
        try {
            ...
            if (!(error instanceof WechatPayApiError && error.code === 'ORDER_NOT_EXIST')) {
                ...
                if (latest.status !== 'pending')
                    return latest;
                console.warn(`[shop] keep expired reservation after close failure id=${latest.id}`, error instanceof Error ? error.message : error);
                return latest;
            }
        }
    }
...
async function closeExpiredActivityOrders(activityId) {
...
shopRouter.post('/activity-registrations/pay', requireShopEnabled, wxPaymentAuth, asyncHandler(async (request, response) => {
    ...
    try {
        ...
        const amount = ACTIVITY_PAYMENT_TEST_AMOUNT_CENTS;
        if (!Number.isSafeInteger(amount) || amount < 0) {
