...
import { callCloudFunction, normalizeComment, normalizePost, } from '../cloudClient.js';
...
async function fetchPostDetail(id) {
    ...
    const comments = Array.isArray(result.data.comments)
        ...
        : [];
    return { success: true, data: { comments, post } };
}
...
postRouter.get('/mine', async (request, response) => {
    ...
    try {
        ...
        const data = result.data
            .map((item) => normalizePost(item))
            .filter((item) => item !== null && item.authorId === openid)
